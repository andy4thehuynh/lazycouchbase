...
module Lazycouchbase
  ...
  class App
    ...
    POLL_TIMEOUT = 0.05
    ACTIONS = %i[
      load_collections load_documents load_indexes open_document open_index
      run_query explain_query refresh yank_document yank_value open_docs
    ].freeze
    ...
    def open_index
      info = @state.selected_index
      return unless info

      @state.show_document(info.name, info.raw)
    end
