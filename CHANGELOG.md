...
- SQL++ snippet library: `tab` in the query editor opens a fuzzy-searchable
  ...
  opener is available.
- Multi-line query editing: `ctrl-j` (or `alt`/`shift`+`enter`) inserts a
  newline and the input box grows with the query, `←`/`→` move the cursor,
  and backspace joins lines, so backspacing at the start of an empty line
  removes it.
